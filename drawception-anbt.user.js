...
// ==/UserScript==
const wrapped = () => {
  const SCRIPT_VERSION = '1.145.2018.10'
  ...
  */
  ...
  const needToGoDeeper = () => {
    ...
    const handleSandboxParameters = () => {
      if (gameinfo.drawingbylink) {
        ...
        const replaylink = `<a href="http://grompe.org.ru/drawit/#drawception/${location.hash.substr(1)}" title="Public replay link for sharing">Drawing</a>`
  ...
  const toggleLight = () => {
    ...
    if (!inDark) {
      if (!css) {
        ...
        $("img[src='/img/duck-gray.svg']", true).forEach(x => {
      ...
      $("img[src='/img/duck.svg']:not([alt='duck']),img[title='Quack'],img[src='/img/duck.svg'][rel='tooltip']", true).forEach(x => {
  ...
  const betterGame = () => {
    ...
    if (options.ownPanelLikesSecret) {
      const panels = $('.panel-user')
      $('.panel-user').forEach(x => {
        const userLink = x.querySelector(`a[href*="/${userid}/"]`)
        if (!userLink) return
        const numlikes = userLink.parentNode.parentNode.querySelector('span.disabled .numlikes')
  ...
  const viewMyGameBookmarks = () => {
    ...
    for (let id in games) {
      ...
      if (id.length > 10) {
        ...
        xhr.send()
      } else if (id.length == 10) result += `<p class="well${extraClass}" id="${id}"><a href="${games[id].url}">${games[id].title}</a>${removeButtonHTML}</p>` // game ID
    }
  ...
  const betterForum = () => {
      ...
      if ((m = tx.match(/^\s*\(last post (...) (\d+).. (\d+):(\d+)([ap]m)\)\s*$/))) {
        ...
        if (location.href.match(/forums\/$/)) {
          if (time != localStorage.getItem(`anbt_subforum${index}`)) {
            x.parentNode.insertAdjacentHTML('afterbegin', '<span class="label label-sm label-warning">NEW</span> ')
    ...
    if (pagination.length) $('.breadcrumb').insertAdjacentHTML('afterend', `<div class="text-center">${pagination[0].outerHTML}</div>`)
  ...
  const pageEnhancements = () => {
    ...
    try {
      ...
      if (runtimever !== '6cf710cd') versionDisplay += '*!!!' // didn't break with one update, hurray
    } catch (e) {
      versionDisplay = `ANBT v${SCRIPT_VERSION}`
    }
    const h = $('#header-bar-container')
    if (h) h.appendChild($(`<div id="anbtver">${versionDisplay}</div>`))
  ...
  if (parseInt(localStorage.getItem('gpe_inDark'), 10) == 1) $("img[src='/img/duck-gray.svg']", true).forEach(x => x.setAttribute('src', '/img/duck.svg'))
