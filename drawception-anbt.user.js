...
// @downloadURL  https://raw.githubusercontent.com/EnderDragonneau/Drawception-ANBT-fix/master/script.js
...
function wrapped() {
    ...
    */
    ...
    function likePanelById(id) {
        $.ajax({
            url: '/game/like/panel.json?panelid=' + id + '&action=Like'
        });
    }
    ...
    function betterGame() {
        ...
        function betterComments() {
            ...
            if (holders.length) {
                ...
                if (maxseenid) seenComments[gameid] = {
                    h: hour,
                    id: maxseenid
                };
    ...
    function betterPanel() {
        ...
        if ($(".btn-primary").last().text() == "Play again") {
            ...
            ccButton.click(function (e) {
                ...
                $.cookie('covercreatorids', JSON.stringify(ids), {
                    expires: 365,
                    path: '/'
                });
    ...
    var panelPositions = {
        ...
        clear: function (page) {
            function clearKeys(obj, keys) {
                $.each(obj, function (k) {
                    if (keys.indexOf(k) < 0) delete obj[k];
                });
            }
    ...
    function viewMyCover() {
        ...
        $.ajax({
            ...
            $("#anbt_userpage").on("click", ".anbt_paneldel", function (e) {
                ...
                panels.remove(id);
                $.cookie('covercreatorids', JSON.stringify(panels), {
                    expires: 365,
                    path: '/'
                });
            });
    ...
    function betterPlayer() {
        ...
        if (loc.match(new RegExp('/player/' + userid + '/[^/]+/(?:$|#)'))) {
            ...
            DrawceptionPlay.deleteCover = function () {
                apprise('Delete the whole cover, really?', {
                    'verify': true
                }, function (r) {
                    if (r) {
                        old_deleteCover();
    ...
    function betterForum() {
        ...
        if (document.location.pathname.match(/\/forums\/(\w+)\/$/)) {
            ...
            tempUnhideLink.text(hidden);
            tempUnhideLink.click(function () {
                $("#main").toggleClass("anbt_showt");
            });
