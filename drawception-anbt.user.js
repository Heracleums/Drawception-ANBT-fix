  ...
  */
  ...
  const setupNewCanvas = (insandbox, url, origpage) => {
    ...
    if (!canvasHTML || canvasHTMLver < NEWCANVAS_VERSION || canvasHTML.length < 10000) {
      ...
      xhr.open('GET', 'https://api.github.com/repos/EnderDragonneau/Drawception-ANBT-fix/contents/newcanvas_embedding.html?ref=test')
    ...
    const // Save friend game id if any
      friendgameid = url.match(/play\/(.+)\//)

    const panelid = url.match(/sandbox\/#?([^\/]+)/)

    const // Handle drawing contests only
      incontest = url.match(/contests\/play\//) && document.getElementById('canvas-holder')
  ...
  const setCookie = (name, value) => (document.cookie = `${name}=${JSON.stringify(value)}`)
  ...
  const linkifyNodeText = (node) => {
    ...
    t.innerHTML = t.innerHTML.replace(/([^"]|^)(https?:\/\/(?:(?:(?:[^\s<()]*\([^\s<()]*\))+)|(?:[^\s<()]+)))/g, '$1<a href="$2">$2</a>')
  }
  ...
  const likePanelById = (id) => {
    ajax(`/game/like/panel.json?panelid=${id}&action=Like`)
  }*/
  ...
  const toggleLight = () => {
    ...
    if (!inDark) {
      if (!css) {
        ...
        $("img[src='/img/duck-gray.svg']").forEach((x) => {
      ...
      $("img[src='/img/duck.svg']:not([alt='duck']),img[title='Quack'],img[src='/img/duck.svg'][rel='tooltip']").forEach((x) => {
  ...
  const fixLocationToCanonical = (m) => {
    let ogurl = $('meta[property="og:url"]').getAttribute('content')
  ...
  const betterGame = () => {
    ...
    const tryNextPanel = function () {
      if (!this.naturalWidth && !this.triedFixing) {
        let pos = this.src.match(/-(\d+)\.png$/)[1]
        pos++
    ...
    drawings.forEach((x) => x.addEventListener('error', tryNextPanel))

    // Hide your own number of likes
    if (options.ownPanelLikesSecret) {
      const numlikes = $('.panel-user')
        .querySelector(`a[href*="/${userid}/"]`)
        .parentNode.parentNode.querySelector('span.disabled .numlikes')
      numlikes.textContent = '?'
      numlikes.style.opacity = '0.5'
    }
    ...
    //.insertAdjacentHTML('afterend', ' <a href="#" class="btn btn-default" onclick="return likeAll();" title="Like all panels"><span class="fas fa-thumbs-up"></span> Like all</a>');

    // Panel favorite buttons
    const favButton = $('<span class="panel-number anbt_favpanel fas fa-heart text-muted" title="Favorite"></span>')
    $('.panel-number').forEach((x) => {
      console.log("test")
      x.insertAdjacentHTML('afterend', favButton.outerHTML)
    })
    $('.gamepanel').forEach((x) => {
      console.log("anbt")
    ...
    $('.anbt_favpanel').forEach((x) => {
      x.addEventListener('click', (e) => {
        ...
        console.log(tp)
        console.log(tp.querySelector('.gamepanel-tools>a:last-child'))
        ...
        //panel.link = tp.querySelector(".gamepanel-tools a:last-child")[0].href.match(/\/panel\/[^\/]+\/([^\/]+)\/[^\/]+\//)[0];
        panel.userLink = tp.querySelector('.panel-user a').href.match(/\/player\/[^\/]+\/[^\/]+\//)[0]
        const img = tp.querySelector('.gamepanel img')
        console.log(img)
        if (img) {
          // Drawing panel
          panel.image = img.getAttribute('src')
          panel.caption = img.getAttribute('alt')
        } else {
          // Caption panel
    ...
    if (options.newCanvas) {
      const addReplayButton = () => {
        ...
        const src = this.src
        checkForRecording(this.src, () => {
          let id
          const newid = src.match(/(\w+).png$/)[1]
          if (newid.length > 8) {
            id = newid
          } else {
            id = scrambleID(panel.getAttribute('id').slice(6))
          }
      ...
      drawings.forEach((x) => x.addEventListener('load', addReplayButton));
      [...drawings].forEach(function () {
    ...
    const betterComments = () => {
      ...
      const gamePlayers = []
      const playerdata = {}
      $('.gamepanel-holder').forEach((i) => {
        const t = i
        const det = t.querySelector('.panel-details')
        const gamepanel = t.querySelector('.gamepanel')
        ...
        playerdata[id] = {
          panel_number: i + 1,
          player_anchor: a[0],
          panel_id: gamepanel.getAttribute('id'),
        ...
        holders.forEach(function (x) {
          ...
          if (vue) {
            ...
            if (vue.edit_date > 0) {
              ...
              const text2 = el.getAttribute('title')
          ...
          const anchorid = t.getAttribute('id')
      ...
      for (let i = 0; i < gamePlayers.length; i++) {
        ...
        if (data.comments != 0) {
          ...
          $(data.player_anchor).insertAdjacentHTML('afterend', `<sup title="${cmt2}">${data.comments}</sup>`)
        }
      }
      if (options.maxCommentHeight) {
        ...
        comments.forEach((x) =>
          x.addEventListener('click', (x) => {
            const t = x
            if (t.clientHeight > h - 50 && !$(location.hash).has(t).length) location.hash = `#${t.parentNode.parentNode.getAttribute('id')}`
          }),
        )
      }
    };
    ...
    const waitForComments = () => {
      ...
      if (comments && !comments[0].classList.contains('spinner')) {
        betterComments()
      } else {
  ...
  const betterPanel = () => {
    ...
    let panels = localStorage.getItem('gpe_panelFavorites')
    panels = panels ? JSON.parse(panels) : {}
    if (panels[document.location.href.match(/\/panel\/[^\/]+\/([^\/]+)\//)[1]] !== undefined) {
      favBtn.setAttribute('disabled', 'disabled')
      favBtn.querySelector('b').textContent = 'Favorited!'
    }
    const img = $('.gamepanel img')
    const d = panelUrlToDate(img.getAttribute('src'))
    if (img && d) {
      $('#main .lead').firstChild.appendChild(`<br>made on ${d}`)
    }
    /*=============================================*/
    ...
    if (options.newCanvas && unscrambleID(panelId) >= 14924553) {
      const img = $('.gamepanel img')
      if (img.length) {
        checkForRecording(img.getAttribute('src'), () => {
          const replayLink = $(`<a class="btn btn-primary" style="margin-top: 20px" href="/sandbox/#${panelId}"><span class="fas fa-redo-alt"></span> <b>Replay</b></a> `)
    ...
    if ([...$('.btn-primary').children].reverse()[0].textContent === 'Play again') {
      // Allow adding to cover creator
      const ccButton = $('<button class="btn btn-info" style="margin-top: 20px"><span class="fas fa-plus"></span> <b>Add to Cover Creator</b></button>')
      ccButton.addEventListener('click', function (e) {
        ...
        if (!ids.includes(id)) {
          ...
          this.setAttribute('disabled', 'disabled').querySelector('b').textContent = 'Already added!'
          return
        }
        setCookie('covercreatorids', JSON.stringify(ids))
        this.setAttribute('disabled', 'disabled').querySelector('b').textContent = 'Added!'
      })
      ...
      if (!panelPositions.player[panelId]) {
        ...
        xhr.onreadystatechange = (html) => {
          if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            html = html.replace(/<img\b[^>]*>/gi, '') // prevent image preload
            ...
            $('.regForm > .lead')
              .appendChild('<br>')
              .appendChild(($('<span>').textContent = panelProgressText))
          }
          xhr.send()
        }
      }
  ...
  const panelPositions = {
    ...
    clear(page) {
      ...
      const existingIds = $(page)
        ...
        .map(function () {
          return getPanelId(this.previousElementSibling.getAttribute('href'))
        })
  ...
  const initAjaxRetry = () => {
    ...
    /*$.ajaxPrefilter(function (options, originalOptions) {
      ...
      const isComment = options.url === "/game/comments/add.json";
      let retryCount = 0;
      ...
      options.success = function (data, textStatus, jqXHR) {
        if (options.url === "/game/like/panel.json" && data && data.error === "Invalid request. You already liked this?") {
          data = {
            callJS: "updateLikeDisplay",
            data: {
              panelid: originalOptions.data.panelid,
              setstatus: originalOptions.data.action === "Like" ? "on" : "off"
            }
          };
        }
  ...
  const viewMyPanelFavorites = () => {
    ...
    $('#anbt_userpage .anbt_paneldel').forEach((x) =>
  ...
  const viewMyGameBookmarks = () => {
    const removeButtonHTML = '<a class="anbt_gamedel pull-right lead fas fa-times btn btn-sm btn-danger" href="#" title="Remove" style="margin-left: 10px"></a>'
    let games = localStorage.getItem('gpe_gameBookmarks')
    games = games ? JSON.parse(games) : {}
    let result = ''
    for (let id in games) {
      ...
      if (id.length > 10) {
        ...
        xhr.onload = () => {
          if (xhr.status === 200) {
            ...
            const spanId = $(`#${id}`)
              .setAttribute('id', gameid)
              .querySelector('span')
            spanId.parentNode.replaceChild(`<a href="${url}">${title}</a>`, spanId)
            localStorage.setItem('gpe_gameBookmarks', JSON.stringify(games))
          } else {
  ...
  const betterPlayer = () => {
    ...
    const pubinfo = $('.profile-header-info .text-mutad:last-child').parentNode
    if (pubinfo) {
    ...
    if (loc.match(new RegExp(`/player/${userid}/[^/]+/(?:$|#)`))) {
      const a = $('<h2>ANBT stuff: </h2>')
      a.appendChild($('<a class="btn btn-primary" href="#anbt_panelfavorites" onclick="viewMyPanelFavorites();">Panel Favorites</a>'))
      a.appendChild($('<a class="btn btn-primary" href="#anbt_gamebookmarks" onclick="viewMyGameBookmarks();">Game Bookmarks</a>'))
      const profilemain = $('.profile-layout-content').firstChild
      profilemain.insertAdjacentHTML('afterbegin', `<h5 id="anbt_userpage">${randomGreeting()}</h5>`)
      ...
      /*if (options.rememberPosition) {
        ...
        $(".progress-striped").forEach(function () {
          const panelId = getPanelId(this.previousElementSibling.getAttribute("href"));
      ...
      if (options.newCanvas) {
        ...
        drawings.forEach((x) => x.addEventListener('load', addReplaySign))
        drawings.forEach(function () {
      ...
      drawings.forEach(({
        ...
        if (src.match(/-1\.png$/)) {
          const drawFirstSign = $('<span class="pull-right" title="Draw First game"><img src="/img/icon-coins.png"></span>')
    ...
    if (loc.match(/player\/\d+\/[^/]+\/(posts)|(comments)\//)) {
      ...
      $('.forum-thread-starter').forEach((x) => {
  ...
  const betterForum = () => {
    const ncPosts = []
    $('span.muted, span.text-muted, small.text-muted').forEach((x, index) => {
      let year
      let month
      let day
      let minutes
      ...
      if ((m = tx.match(/^\s*\(last post (...) (\d+).. (\d+):(\d+)([ap]m)\)\s*$/))) {
        ...
        t.textContent = `(last post ${time})`
        // Track new posts at subforum list
        if (location.href.match(/forums\/$/)) {
          if (time != localStorage.getItem(`anbt_subforum${index}`)) {
            t.parentNode.insertAdjacentHTML('afterbegin', '<span class="label label-sm label-warning">NEW</span> ')
    ...
    if (options.markStalePosts) {
      ...
      for (let i = 1; i < ncPosts.length; i++) {
        ...
        if (age > 30) {
          markStalePost($(el).parentNode.parentNode.parentNode.parentNode, age)
        }
      }

      GM_addStyle(".anbt_necropost:after {display: block; height: 14px; border-bottom: 2px solid black; content: ' '; background: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAsAAAAOCAMAAADOvxanAAAAElBMVEUAAAD///8wLiSYlYNnZFXm5Nfd4sMOAAAAAnRSTlMAAHaTzTgAAABBSURBVHheXchBEoBADAJBMsD/v2yZdS8Oly40k9NIk54ySm/8DYb6NWuuex1ak7VdbAcpNrCU8HmP4/hzd9oAXj6sBgHBLAHrRAAAAABJRU5ErkJggg==)}" + '.anbt_necropost0:after {background: none; border-bottom: 1px solid black}' + '.anbt_necropost1:after {background: none}' + '.anbt_necropost2:after {background-repeat: no-repeat; background-position: center}' + '.anbt_necropost3:after {}' + ".anbt_necropost span.muted:after {content: ' (old post)'}" + 'iframe {border: none}' + '.v--modal-overlay .v--modal-background-click {padding-bottom: 0}')
    }
    // Linkify the links
    $('.comment-body *').forEach((x) => {
      linkifyNodeText(x)
    })

    // Linkify drawing panels
    $('img[src*="/images/panels/"], img[src*="/pub/panels/"]').forEach((x) => {
      const t = x
      if (!t.parentNode.nodeName.toLowerCase() === 'a') {
        const gameid = t.getAttribute('src').match(/\/([^-]+)-\d+.png/)[1]
        const gameurl = `/game/${gameid}/-/`
        t.wrap(`<a href="${gameurl}"></a>`)
      }
    })
    $('img[src*="/drawings/"]').forEach((x) => {
      const t = x
      if (!t.parentNode.nodeName.toLowerCase() === 'a') {
        const panelid = t.getAttribute('src').match(/(\w+).png$/)[1]
        const panelurl = `/panel/drawing/${panelid}/-/`
        t.wrap(`<a href="${panelurl}"></a>`)
      }
    })
    $('img[src*="/panel/"]').forEach((x) => {
      const t = x
      if (!t.parentNode.nodeName.toLowerCase() === 'a') {
        t.wrap(`<a href="${t.getAttribute('src')}-/"></a>`)
      }
    })
    // Linkify full game image
    $('img[src*="/images/games/"], img[src*="/pub/games/"]').forEach((x) => {
      const t = x
      if (!t.parentNode.nodeName.toLowerCase() === 'a') {
        const gameid = t.getAttribute('src').match(/\/([^\/]+)\.png/)[1]
        const gameurl = `/game/${gameid}/-/`
        t.wrap(`<a href="${gameurl}"></a>`)
      }
    })
    // Fix the dead link
    $('img[src*="/display-panel.php?"]').forEach((x) => {
      const t = x
      if (!t.parentNode.nodeName.toLowerCase() === 'a') {
        const panelid = t.getAttribute('src').match(/x=(\d+)/)[1]
        const newsrc = `/panel/drawing/${scrambleID(panelid)}/`
    ...
    if (document.location.pathname.match(/\/forums\/(\w+)\/.+/)) {
      ...
      $('.comment-avatar').forEach(({
        ...
        if (vue) {
          ...
          if (vue.edit_date > 0) {
            const el = ts.parentNode.querySelector('span[rel="tooltip"]')
            let text2 = el.getAttribute('title')
            text2 += `, ${formatTimestamp(vue.edit_date * 1000).replace(/ /g, '\u00A0')}` // prevent the short tooltip width from breaking date apart
            el.setAttribute('title', text2)
          }
        }
        if (anch) {
          ...
          try {
            const h = t.querySelector('a[href^="/player/"]').getAttribute('href')
            if (h) {
      ...
      if ($('.comment-holder').length == 20) {
        $('#comment-form .btn-primary').insertAdjacentHTML('afterend', '<div>Note: posting to another page</div>')
      }
    }

    if (options.proxyImgur) {
      $('img[src*="imgur.com/"]').forEach(function () {
        const t = this
        // Google Proxy has died.
        //t.getAttribute("src", "http://www.gmodules.com/ig/proxy?url=" + encodeURIComponent(t.getAttribute("src")));
        t.setAttribute('src', t.getAttribute('src').replace('imgur.com', 'filmot.com'))
      })
    }

    const pagination = $('.pagination')
    if (pagination.length) {
      const e = pagination.cloneNode(true)
      $('.breadcrumb').insertAdjacentHTML('afterend', e.outerHTML)
    ...
    if (document.location.pathname.match(/\/forums\/(\w+)\/$/)) {
      ...
      $('.forum-thread').forEach((x) => {
        ...
        hideLink.addEventListener('click', () => {
          ...
          if (hidden_topics.includes(id)) {
            ...
            t.classList.remove('anbt_hidden')
            hidden--
          } else {
        ...
        t.querySelector('p:nth-child(2)').appendChild(hideLink)
      })
      tempUnhideLink.textContent = hidden
      ...
      $('.forum-thread').forEach(({
        firstChild
      }) => firstChild.insertAdjacentHTML('beforebegin', tempUnhideLink.outerHTML))
    }
    $('.btn.btn-default').forEach((x) =>
      x.addEventListener('click', function (e) {
        if (this.textContent === 'Draw beta') {
  ...
  const addScriptSettings = () => {
    const theForm = $('<form class="regForm form-horizontal" action="#" onsubmit="return updateScriptSettings(this);"></form>')
    theForm.appendChild($('<legend>ANBT script settings</legend>'))

    const addGroup = (name, settings) => {
      const div = $('<div class="control-group"></div>')
      div.appendChild($(`<label class="control-label">${name}</label>`))
      settings.forEach((id) => {
        const v = options[id[0]]
        const name = id[0]
        const t = id[1]
        const desc = id[2]
        const c = $('<div class="controls"></div>')
        if (t == 'boolean') {
          c.appendChild($(`<label><input type="checkbox" id="anbt_${name}" name="${name}" value="1" ${v ? 'checked="checked"' : ''}"> ${desc}</label>`))
        } else if (t == 'number') {
          if (!v) v = 0
          $(`<b>${desc}:</b><input class="form-control" type="text" data-subtype="number" name="${name}" value="${escapeHTML(v)}">`).forEach((x) => c.appendChild(x))
        } else if (t == 'longstr') {
          $(`<b>${desc}:</b><textarea class="form-control" name="${name}">${escapeHTML(v)}</textarea>`).forEach((x) => c.appendChild(x))
        } else {
          $(`<b>${desc}:</b><input class="form-control" type="text" name="${name}" value="${escapeHTML(v)}">`).forEach((x) => c.appendChild(x))
        }
        div.appendChild(c)
      })
  ...
  const autoSkip = (reason) => {
    const autoSkipInfo = $(`<div id="autoSkipInfo" class="text-warning" style="cursor: pointer">(CLICK TO CANCEL)<br>Auto-skipping in <span id="autoSkipCounter">3</span>...<br>Reason: ${reason}</div>`)
  ...
  const pagodaBoxError = () => {
    if (document.title == 'Pagoda Box' && (document.body.innerHTML.match('All Routes' + ' Down.') || document.body.innerHTML.match('There appears to be an error' + ' with this site.'))) {
      ...
      div.innerHTML = '<h1>ANBT speaking:</h1>' + 'Meanwhile, you can visit the chat: ' + '<a href="http://chat.grompe.org.ru/#drawception">http://chat.grompe.org.ru/#drawception</a><br>' + 'Or use the new sandbox: <a href="http://grompe.org.ru/drawit/">http://grompe.org.ru/drawit/</a>'
  ...
  const pageEnhancements = () => {
    ...
    try {
      ...
      userid = tmpuserlink.getAttribute('href').match(/\/player\/(\d+)\//)[1]
    ...
    if (options.maxCommentHeight) {
      ...
      $('.comment-body').forEach((x) =>
        x.addEventListener('click', () => {
          const t = x
          if (t.clientHeight > h - 50 && location.hash.indexOf(t) === -1) location.hash = `#${t.parentNode.parentNode.getAttribute('id')}`
        }),
      )
    }
    ...
    p.appendChild($('<span class="gpe-wide gpe-spacer"></span>'))
    p.appendChild($('<a href="/sandbox/" title="Sandbox" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item" style="background:#5A5"><span class="fas fa-edit" style="color:#BFB" /></a>'))
    p.appendChild($('<a href="/browse/all-games/" title="Browse Games" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item"><span class="fas fa-folder-open" /></a>'))
    p.appendChild($('<a href="/contests/" title="Contests" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item"><span class="fas fa-trophy" /></a>'))
    p.appendChild($('<a href="javascript:toggleLight()" title="Toggle light" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item" style="background:#AA5"><span class="fas fa-eye" style="color:#FFB" /></a>'))
    p.appendChild($('<a href="/leaderboard/" title="Leaderboards" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item"><span class="fas fa-fire" /></a>'))
    p.appendChild($('<a href="/faq/" title="FAQ" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item"><span class="fas fa-question-circle " /></a>'))
    p.appendChild($('<a href="/forums/" title="Forums" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item" style="background:#55A"><span class="fas fa-comments" style="color:#BBF" /></a>'))
    p.appendChild($('<a href="/search/" title="Search" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item"><span class="fas fa-search" /></a>'))
    p.appendChild($('<a id="menusettings" href="/settings/" title="Settings" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item"><span class="fas fa-cog" /></a>'))
    p.appendChild($('<a href="/logout" title="Log Out" class="gpe-wide gpe-btn btn btn-menu navbar-btn navbar-user-item" style="background:#A55"><span class="fas fa-sign-out-alt" style="color:#FBB" /></a>'))
    // Let users with screens narrow enough so top bar isn't visible still use toggle light function
    $('#main-menu').insertAdjacentHTML('afterbegin', '<a href="javascript:toggleLight()" class="list-group-item"><span class="fas fa-eye"></span> Toggle light</a>')

    p = $('.btn-menu-player').parentNode
    const userlink = $('.player-dropdown a[href^="/player/"]').getAttribute('href')
    const useravatar = $('.btn-menu-player').innerHTML
    const element = $(`<a href="${userlink}" title="View Profile" class="gpe-wide-block navbar-btn navbar-user-item" style="margin-top:8px">${useravatar}</a>`)
    ...
    const num = $('#user-notify-count').textContent.trim()
    ...
    window.getNotifications = () => {
      if (!notificationsOpened) {
        $('#user-notify-list').innerHTML = '<img src="/img/loading.gif" alt="Loading...."/>'
        const xhr = new XMLHttpRequest()
    ...
    let versionDisplay
    try {
      const appver = $('script[src^="/build/app"]')
        .getAttribute('src')
        .match(/(\w+)\.js$/)[1]
      const runtimever = $('script[src^="/build/runtime"]')
        .getAttribute('src')
        .match(/(\w+)\.js$/)[1]
      versionDisplay = `ANBT v${SCRIPT_VERSION} | app ${appver}`
      if (appver !== SITE_VERSION) versionDisplay += '*'
      versionDisplay += ` | runtime ${runtimever}`
      if (runtimever !== '6daa7d0a') versionDisplay += '*!!!' // didn't break with one update, hurray
    } catch (e) {
      versionDisplay = `ANBT v${SCRIPT_VERSION}`
    }
    $('#header-bar-container').appendChild($(`<div id="anbtver">${versionDisplay}</div>`))

    $('.footer-main .list-unstyled')[0].appendChild($('<li><a href="/forums/-/11830/-/">ANBT script</a></li>'))
    $('.footer-main .list-unstyled')[1].appendChild($('<li><a href="http://drawception.wikia.com/">Wiki</a></li>'))
    $('.footer-main .list-unstyled')[2].appendChild($('<li><a href="http://chat.grompe.org.ru/#drawception">Chat</a> (<a href="https://discord.gg/CNd5KTJ">Discord</a>)</li>'))

    if (options.newCanvas) {
      let directToNewSandbox = function (e) {
        if (e.which === 2) return
        e.preventDefault()
        setupNewCanvas(true, this.href)
      }
      let directToNewPlay = function (e) {
        if (e.which === 2) return
        e.preventDefault()
        setupNewCanvas(false, this.href)
      }
      $('a[href^="/sandbox/"]').forEach((x) => x.addEventListener('click', directToNewSandbox))
      $('a[href="/play/"]').addEventListener('click', directToNewPlay)
    }
  }
